...
percept = lpips.PerceptualLoss(model='net', net='vgg', use_gpu=True)
...
def reconstruct(args, config_file):
    ...
    for iteration in range(5001):
        ...
        rec_loss = (percept(xray_recons[0], target_xray).sum() / sigma1) + scalar1 +\
                 (F.mse_loss(xray_recons[0], target_xray) / sigma2) + scalar2
