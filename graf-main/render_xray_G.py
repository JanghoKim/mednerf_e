...
percept = lpips.PerceptualLoss(model='net', net='vgg', use_gpu=True) # model='net-lin', net='alex'
...
def test(range_phi, render_radius, theta_mean,
    ...
    with torch.no_grad():
        ...
        for i in range(N_samples):
            save_video(rgb[i], os.path.join(eval_dir,
...
def reconstruct(args, config_file):
    ...
    for iteration in range(5000):
        ...
        scalar1 = torch.tensor(0.0, requires_grad=True)
        scalar2 = torch.tensor(0.0, requires_grad=True)
        sigma1 = torch.exp(scalar1)
        sigma2 = torch.exp(scalar2)

        rec_loss = (percept(torch.unsqueeze(xray_recons[0][0],0),
                    target_xray).sum() / sigma1) + scalar1 +\
                 (F.mse_loss(torch.unsqueeze(xray_recons[0][0],0), target_xray) / sigma2) + scalar2
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_size', type=int, default=80, help='Use a size of 64 if GPU size <=10GB ')
